# 🐱 Kitty - 技术实施方案 v3.0（已归档）

> ⚠️ **本文档已归档。** 当前版本的完整技术文档请参阅 [技术文档.md](./技术文档.md)（v4.0，基于 LangGraph Agent 实际实现编写）。
>
> 本文档记录的是 v3.x 架构（基于 OpenClaw 代理），已被 v4.0 的 LangGraph Agent 方案替代。
> 仅作历史参考保留。
...
| `/chat` | POST | 对话，支持流式(SSE)/非流式(JSON)，支持模型切换 |
| `/tts` | POST | 语音合成，接收文本，返回音频 |
| `/models` | GET | 获取可用模型列表（从 OpenClaw WS 动态获取，5min 缓存） |
| `/session/{id}` | GET | 获取会话历史 |
| `/session/{id}` | DELETE | 清空会话 |

**模型切换机制**：OpenClaw HTTP API 的 `model` 字段只接受 `openclaw/openclaw/<agentId>` 格式，不支持 `provider/model`。模型切换通过 `x-openclaw-model` header 实现，客户端传入的模型 ID 通过此 header 传递给 OpenClaw 路由到对应底层模型。
...
│   ├── Kitty/
│   │   ├── App/
│   │   │   ├── KittyApp.swift    # 入口（TabView: 语音通话 + 文字聊天）
│   │   │   └── AppDelegate.swift # 音频会话配置
│   │   ├── Config/
│   │   │   └── APIConfig.swift   # 配置、ModelInfo 定义
│   │   ├── Services/
│   │   │   ├── KittyService.swift        # 后端 API 调用（ASR/Chat/TTS/Models）
│   │   │   ├── ConversationManager.swift # 语音对话管理
│   │   │   ├── ChatManager.swift         # 文字聊天管理
│   │   │   └── MessageStore.swift        # 聊天消息持久化
│   │   ├── Views/
│   │   │   ├── CallView.swift    # 通话界面
│   │   │   ├── ChatView.swift    # 聊天界面
...
│   ├── main.py                   # FastAPI 服务
│   ├── requirements.txt          # Python 依赖（fastapi, uvicorn, httpx, websockets, pynacl）
...
uv pip install fastapi uvicorn httpx websockets pynacl
```
...
```json
{
    ...
    "history": [],
    "stream": false,
    "model": "qianfan/deepseek-v4-flash",
    "session_id": "default"
}
```
...
**模型切换**：通过 `model` 参数指定底层模型，服务端通过 `x-openclaw-model` header 传递给 OpenClaw。
