...
| LLM 调用 | `httpx` 直接调用 | 项目已有 models.json 多 Provider 体系 |
...
> **LangGraph 只是 Workflow Runtime。真正的壁垒在 Context Engine + Memory Strategy + State Management。而这些，只有自己掌控代码才能做好。**

V1 只做一件事：把 `/chat` 端点从 `httpx.post(OpenClaw)` 替换为 `graph.ainvoke()`，其他一切不变。这是最小可行替换，风险最低，验证最快。
